...
public:
    ...
    IOSOpenClosedData(const state &theData, double fCost, double gCost, double hCost, uint64_t parent, uint64_t openLoc,
...
public:
    ImprovedOptimisticSearch() {
        ...
        weight = 3;
        bound = 1.5;
        theOptimalHeuristic = 0;
        theGreedyHeuristic = 0;
    ...
    inline const IOSOpenClosedData<state> &GetItem(unsigned int which) { return openClosedList.GetItem(which); }

    bool HaveExpandedState(const state &val) {
        uint64_t key;
        return openClosedList.Lookup(env->GetStateHash(val), key) != kNotFound;
    }

    dataLocation GetStateLocation(const state &val) {
        uint64_t key;
        return openClosedList.Lookup(env->GetStateHash(val), key);
    }
    ...
    void SetOptimalHeuristic(Heuristic<state> *h) { theOptimalHeuristic = h; }

    void SetGreedyHeuristic(Heuristic<state> *h) { theGreedyHeuristic = h; }
...
private:
    ...
    Heuristic<state> *theOptimalHeuristic;
    Heuristic<state> *theGreedyHeuristic;
    bool doneGreedy;
    double maxPriority;
};
...
template<class state, class action, class environment>
void
ImprovedOptimisticSearch<state, action, environment>::GetPath(environment *_env, const state &from, const state &to,
 ...
 */
template<class state, class action, class environment>
bool ImprovedOptimisticSearch<state, action, environment>::InitializeSearch(environment *_env, const state &from,
                                                                            const state &to,
                                                                            std::vector<state> &thePath) {
    doneGreedy = false;
    if (theGreedyHeuristic == 0)
        theGreedyHeuristic = _env;
    if (theOptimalHeuristic == 0)
        theOptimalHeuristic = _env;
    thePath.resize(0);
    env = _env;
    ResetNodeCount();
    start = from;
...
//	phi = ([=](double x, double y){return y/(w)+x;});
    ...
    double h = theGreedyHeuristic->HCost(from, to);
...
void ImprovedOptimisticSearch<state, action, environment>::AddAdditionalStartState(state &newState) {
    double h = theGreedyHeuristic->HCost(newState, goal);
    ...
    for (int x = 0; x < neighbors.size(); x++) {
        ...
        switch (neighborLoc[x]) {
            ...
            case kOpenList:
                ...
                break;
            case kNotFound: {
                double h = theGreedyHeuristic->HCost(neighbors[x], goal);
    ...
    for (int x = 0; x < neighbors.size(); x++) {
        ...
        switch (neighborLoc[x]) {
            ...
            case kOpenList:
                ...
                break;
            case kNotFound: {
                double h = theOptimalHeuristic->HCost(neighbors[x], goal);
                openClosedList.AddOpenNode(neighbors[x],
                                           env->GetStateHash(neighbors[x]),
                                           phi(h, openClosedList.Lookup(nodeid).g + edgeCosts[x]),
                                           openClosedList.Lookup(nodeid).g + edgeCosts[x],
